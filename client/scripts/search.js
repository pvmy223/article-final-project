document.addEventListener("DOMContentLoaded", async () => {
  ...
  const resultsContainer =
    ...
    document.querySelector("#searchResults.space-y-6");
  ...
  async function performSearch() {
    try {
        ...
        if (searchQuery) {
            if (tags.length > 0) {
                const tagElements = document.querySelectorAll('input[type="checkbox"]:checked');
                const tagNames = Array.from(tagElements).map(el => el.nextElementSibling.textContent.trim());
                searchQuery.textContent = `Tags: ${tagNames.join(', ')}`;
            } else {
                searchQuery.textContent = query || "tất cả";
            }
        }

        const result = await SearchService.searchArticles(query, page, category, tags);
      ...
      const renderedHTML = result.articles
        .map(
          (article) => `
